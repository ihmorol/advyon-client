...
let parts = content.split('<div className="border rounded-lg overflow-x-auto">');
if (parts.length > 1) {
    // 0 is before, 1 is after Users table, 2 is after Recent Cases table (if matched), 3 is after Audit Logs (if any)
    content = content.replace(
